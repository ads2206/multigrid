...
mg_grid = MultiGridClass(x, U0, domain, f, solver='SOR')
gs_grid = MultiGridClass(x, U0, domain, f, solver='GS')
sor_grid = MultiGridClass(x, U0, domain, f, solver='SOR')


iterations = [0]
error = [(mg_grid.get_error(u_true), gs_grid.get_error(u_true), sor_grid.get_error(u_true))]
for i in range(125):
    mg_grid.v_sched()
    gs_grid.iterate(4)
    sor_grid.iterate(4)
    error.append((mg_grid.get_error(u_true), gs_grid.get_error(u_true), sor_grid.get_error(u_true)))
